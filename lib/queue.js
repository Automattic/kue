...
var EventEmitter = require('events').EventEmitter
  ...
  , QueueEvents       = require('./queueEvents')
  , QueueJob          = require('./queueJob')
...
function Queue( options ) {
  ...
  this.redis = redis();
  this.redis.configureFactory(options, this);

  this.client = this.redis.createClient();

  this.events      = QueueEvents(this);

  this.Job = QueueJob(this);
...
Queue.prototype.create = Queue.prototype.createJob = function( type, data ) {
  return this.Job.createJob(type, data);
};
...
Queue.prototype.setupTimers = function() {
  if( this.warlock === undefined ) {
    this.lockClient = this.redis.createClient();
      ...
      if( typeof unlock === 'function' ) {
        ...
        client.zrangebyscore(client.getKey('jobs:delayed'), 0, Date.now(), 'LIMIT', 0, limit, function( err, ids ) {
          ...
          ids.forEach(function( id ) {
            ...
            self.Job.get(id, function( err, job ) {
    ...
    self.warlock.lock('activeJobsTTL', lockTtl, function( err, unlock ) {
      ...
      if( typeof unlock === 'function' ) {
        ...
        client.zrangebyscore(client.getKey('jobs:active'), 100000, Date.now(), 'LIMIT', 0, limit, function( err, ids ) {
          ...
          ids.forEach(function( id ) {
            id = client.stripFIFO(id);
            self.events.emit(id, 'ttl exceeded');
          });
        });
      } else {
        // The lock was not established by us, be silent
...
var testMode = require('./queueTestMode')
Queue.prototype.testMode = function(){
  if(!this._testMode){
    this._testMode = testMode(this.Job.Job)
  }
...
Object.defineProperty(Queue, 'app', {
  get: function() {
