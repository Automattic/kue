...
o = (function(){
  ...
  function fromHTML(html, args) {
    ...
    var type = /^<(\w+)/.exec(html)[1]
      , wrap = map[type] || map.default
      , depth = wrap[0];

    div.innerHTML = wrap[1] + html + wrap[2];
    while (depth--) div = div.lastChild;

    return oQuery(div.firstChild);
  }
  ...
  oQuery.prototype.append = function(val){
    return oQuery(val).appendTo(this);
  };
  ...
  oQuery.prototype.prependTo = function(val){
    // TODO: finish
    if (val = oQuery(val).get(0)) {
      val.insertBefore(this.get(0), val.firstChild);
    }
    return this;
  };
