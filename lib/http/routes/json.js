...
exports.jobRange = function( req, res ) {
  ...
  queue.range(from, to, order, function( err, jobs ) {
    if( err ) return res.json({ error: err.message });
    res.json(jobs);
  });
};
...
exports.jobStateRange = function( req, res ) {
  ...
  queue.rangeByState(state, from, to, order, function( err, jobs ) {
    if( err ) return res.json({ error: err.message });
    res.json(jobs);
  });
};
...
exports.jobTypeRange = function( req, res ) {
  ...
  queue.rangeByType(type, state, from, to, order, function( err, jobs ) {
    if( err ) return res.json({ error: err.message });
    res.json(jobs);
  });
};
...
exports.search = function( req, res ) {
  res.json({error: true, message: 'not supported'});
};
