...
/**
 * Return time series job ids with a given `state` since a given `timestamp`, and callback `fn(err, {hourTimestamp: [ids]})`
 *
 * @param {String} state : either complete or failed
 * @param {Date} timestamp to get the ids since
 * @param {Function} fn
 * @return {Queue} for chaining
 * @api public
 */

Queue.prototype.stateSince = function( state, timestamp, fn ) {
  var self = this;
  var result = {};

  //get all the hours since the timestamp
  var hourTimestamps = [];
  var now = Date.now();

  //floor current timestamp to the closest hour boundry *after* the provided timestamp
  var currentTimestamp = Math.floor((timestamp + 3600000) / 3600000) * 3600000;

  while(currentTimestamp < now) {
    hourTimestamps.push(currentTimestamp);
    currentTimestamp += 3600000;
  }

  async.each(hourTimestamps, function(hourTimestamp, callback){
    this.client.zrange(this.client.getKey('jobs:' + state + ':' + hourTimestamp), 0, -1, function(err, ids) {
      if(err) return callback(err);

      result[hourTimestamp] = [];
      ids.forEach(function(id){
        result[hourTimestamp].push(self.client.stripFIFO(id));
      });
      //success
      callback();
    });
...
/**
 * Return time series job ids with a given `state` at a given `timestamp`, and callback `fn(err, {hourTimestamp: [ids]})`
 *
 * @param {String} state : either complete or failed
 * @param {Date} timestamp to get the ids since
 * @param {Function} fn
...
Queue.prototype.stateAt = function( state, timestamp, fn ) {
  var self = this;
  var result = {};

  console.log(timestamp);

  //floor povided timestamp to the closest hour boundry *before* the provided timestamp
  var hourTimestamp = Math.floor((timestamp) / 3600000) * 3600000;
...
/**
 * Time series collection of completed jobs since given timestamp.
...
Queue.prototype.completeSince = function( timestamp, fn ) {
  return this.stateSince('complete', timestamp, fn);
};

/**
 * Time series collection of completed jobs at given timestamp.
 * @param {Number} timestamp
 * @param {Function} fn
 * @return {Queue} for chaining
 * @api public
...
Queue.prototype.completeAt = function( timestamp, fn ) {
  return this.stateAt('complete', timestamp, fn);
};
...
/**
 * Time series collection of failed jobs since given timestamp.
 * @param {Number} timestamp
 * @param {Function} fn
 * @return {Queue} for chaining
 * @api public
 */
 ...
 Queue.prototype.failedSince = function( timestamp, fn ) {
   return this.stateSince('failed', timestamp, fn);
 };

 /**
  * Time series collection of failed jobs at given timestamp.
  * @param {Number} timestamp
  * @param {Function} fn
  * @return {Queue} for chaining
  * @api public
  */
  ...
  Queue.prototype.failedAt = function( timestamp, fn ) {
    return this.stateAt('failed', timestamp, fn);
  };
