...
/**
 * sets up promotion & stalled timers
 */
...
Queue.prototype.checkJobPromotion = function( promotionOptions ) {
  ...
  this.promoter = setInterval(function() {
    self.redlock
      ...
      .then(function( lock ) {
        return client.zrangebyscore(client.getKey('jobs:delayed'), 0, Date.now(), 'LIMIT', 0, limit, function( err, ids ) {
          if( err || !ids.length ) {
            return lock.unlock();
          }
  ...
  this.stalledJobsTimer = setInterval(function() {
    self.redlock
      ...
      .then(function(lock) {
        ...
        return client.zrangebyscore(client.getKey('jobs:active'), 0, then, 'LIMIT', 0, limit,
          function( err, ids ) {
            if( err || !ids.length ) {
              return lock.unlock();
            }
...
Queue.prototype.process = function( type, n, fn ) {
  ...
  while( n-- ) {
    ...
    worker.on('job complete', function( job ) {
      if( self.client ) {
        self.client._getPipeline().incrby(
          self.client.getKey('stats:work-time'),
          job.duration,
          noop
        );
      }
...
/**
 * Change job processing concurrency for a specific worker type
 *
 * @param {String} type the worker type
 * @param n change worker concurrency to n
 */
Queue.prototype.setConcurrency = function( type, n ) {
