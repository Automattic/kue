...
exports.createQueue = function ( options ) {
    ...
    events.key = Queue.singleton.client.prefix;
    events.subscribe();
...
function Queue( options ) {
    ...
    if( options.redis ){
        redis.createClient = function() {
            ...
            client.prefix = options.redis.prefix || 'q';
            return client;
        }.bind(this);
    } else {
...
Queue.prototype.promote = function (ms, l) {
    ...
    this.promoter = setInterval(function () {
        client.sort(client.prefix+':jobs:delayed'
            , 'by', client.prefix+':job:*->delay'
            , 'get', '#'
            , 'get', client.prefix+':job:*->delay'
            , 'get', client.prefix+':job:*->created_at'
...
Queue.prototype.setting = function (name, fn) {
    this.client.hget(this.client.prefix+':settings', name, fn);
    return this;
};
...
Queue.prototype.process = function (type, n, fn) {
    ...
    while (n--) {
        ...
        worker.on('job complete', function (job) {
            if (self.client) {
                self.client.incrby(self.client.prefix+':stats:work-time', job.duration);
            }
        });
...
Queue.prototype.types = function (fn) {
    this.client.smembers(this.client.prefix+':job:types', fn);
    return this;
};
...
Queue.prototype.state = function (state, fn) {
    this.client.zrange(this.client.prefix+':jobs:' + state, 0, -1, fn);
    return this;
};
...
Queue.prototype.workTime = function (fn) {
    this.client.get(this.client.prefix+':stats:work-time', function (err, n) {
...
Queue.prototype.card = function (state, fn) {
    this.client.zcard(this.client.prefix+':jobs:' + state, fn);
    return this;
};
