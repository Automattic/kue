...
Queue.prototype.promote = function(ms,l){
  ...
  setInterval(function(){
    client.sort(redis.queueName+':jobs:delayed'
      , 'by', redis.queueName+':job:*->delay'
      , 'get', '#'
      , 'get', redis.queueName+':job:*->delay'
      , 'get', redis.queueName+':job:*->created_at'
...
Queue.prototype.setting = function(name, fn){
  this.client.hget(redis.queueName+':settings', name, fn);
  return this;
};
...
Queue.prototype.process = function(type, n, fn){
  ...
  while (n--) {
    ...
    worker.on('job complete', function(job){
      self.client.incrby(redis.queueName+':stats:work-time', job.duration);
    });
...
Queue.prototype.pause = function(type, fn, timeout) {
    var origFn = fn || function(){}
    , self = this
    , n = self.workers.length;
...
Queue.prototype.types = function(fn){
  this.client.smembers(redis.queueName+':job:types', fn);
  return this;
};
...
Queue.prototype.state = function(state, fn){
  this.client.zrange(redis.queueName+':jobs:' + state, 0, -1, fn);
  return this;
};
...
Queue.prototype.workTime = function(fn){
  this.client.get(redis.queueName+':stats:work-time', function(err, n){
...
Queue.prototype.card = function(state, fn){
  this.client.zcard(redis.queueName+':jobs:' + state, fn);
  return this;
};
