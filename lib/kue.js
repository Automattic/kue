...
function Queue( options ) {
    ...
    if( options.redis || options.prefix ){
        ...
        redis.createClient = function() {
            ...
            var client = require('redis').createClient(socket || port , host, options.redis.options );
            if (options.redis.auth) {
                client.auth(options.redis.auth);
            }
            if (options.redis.db) {
                client.select(options.redis.db, function() { console.log("Selected redis database ", options.redis.db, arguments ); });
            }
