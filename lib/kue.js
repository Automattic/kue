...
Queue.prototype.watchStuckJobs = function( ms, ...queues ) {
  var client = this.client
    , self   = this
    , ms     = ms || 1000;
  ...
  if (queues.length === 0) {
    throw new Error(`Refusing to iterate over all queues - please be specific.`);
  }
  ...
  for i=1,#KEYS do
    ...
    local activeQueueName = '${prefix}:' .. queueName .. ':jobs'
    local numInactive = redis.call('zcard', '${prefix}:jobs:' .. queueName .. ':inactive')
    local numPending = redis.call('llen', activeQueueName)

    if numInactive > numPending then
      need_fix = need_fix + 1
      for j=1,(numInactive-numPending) do
        redis.call('lpush', activeQueueName, 1)
      end
    end
  end
  redis.log(redis.LOG_WARNING, 'Found ' .. need_fix .. ' stuck jobs')
  return need_fix
  `;
  ...
  client.script('LOAD', script, function( err, sha ) {
    ...
    this.stuck_job_watch = setInterval(function() {
      client.evalsha(sha, queues.length, ...queues, function( err, fixes ) {
