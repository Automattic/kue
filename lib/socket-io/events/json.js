...
exports.stats = function( send ) {
  ...
  ('workTime')
  (function( err, obj ) {
    if( err ) return send && send({ error: err.message });
    send && send(obj);
  });
};
...
exports.inactive = function( body, send ) {
  ...
  Job.get(id, function( err, job ) {
    if( err ) return send && send({ error: err.message });
    job.inactive();
    send && send({ message: 'job ' + id + ' inactive' });
  });
};
...
exports.remove = function( body, send ) {
  ...
  Job.remove(id, function( err ) {
    if( err ) return send && send({ error: err.message });
    send && send({ message: 'job ' + id + ' removed' });
  });
};
...
exports.updatePriority = function( body, send ) {
  ...
  if( isNaN(priority) ) return send && send({ error: 'invalid priority' });
  Job.get(id, function( err, job ) {
    if( err ) return send && send({ error: err.message });
    job.priority(priority);
    job.save(function( err ) {
      if( err ) return send && send({ error: err.message });
      send && send({ message: 'updated priority' });
    });
  });
};
...
exports.updateState = function( body, send ) {
  ...
  Job.get(id, function( err, job ) {
    ...
    job.save(function( err ) {
      if( err ) return send && send({ error: err.message });
      send && send({ message: 'updated state' });
    });
  });
};
