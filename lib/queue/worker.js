...
Worker.prototype.getJob = function(fn){
  ...
  var client = clients[self.type]
	...
	client.keys('q:jobs:' + self.type + ':inactive', function(err, keys){
		if (err) return fn(err);
		if (!keys.length) return fn();
		// using tail recursion so we can stop the loop when found
		function getFirstJob(index) {
			var key = keys[index];			
			var unique = key.replace('q:jobs:', '').replace(':inactive', '');
			// BLPOP indicates we have a new inactive job to process
			client.blpop('q:' + unique + ':jobs', 0, function(err) {
				if (err) return fn(err);
			    self.zpop('q:jobs:' + unique + ':inactive', function(err, id){
			      if (err) return fn(err);
			      if (id) return Job.get(id, fn);
                  if (keys.length-1 == index) return fn();
                  return getFirstJob(index+1);
			    });
			});
		}
		getFirstJob(0);
