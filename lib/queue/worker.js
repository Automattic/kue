...
Worker.prototype.getJob = function(fn){
  ...
  client.blpop(redis.queueName+':' + self.type + ':jobs', 0, function(err) {
    if (err) return fn(err);		// SAE: Added to avoid crashing redis on zpop
      if( !self.running ) {
          return client.lpush(redis.queueName+':' + self.type + ':jobs', 1, function(){fn(err||"shutdown pushback");});
      }
    // Set job to a temp value so shutdown() knows to wait
    self.job = true;
    self.zpop(redis.queueName+':jobs:' + self.type + ':inactive', function(err, id){
      if (err || !id ) {
          self.job = false;
...
Worker.prototype.shutdown = function(fn, timeout) {
  ...
  var _fn = function(job) {
      ...
      setTimeout( function(){ self.client.lpush(redis.queueName+':' + self.type + ':jobs', 1, function(){}); }, 2000 );
