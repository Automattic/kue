...
function Worker( queue, type ) {
  this.queue   = queue;
  this.type    = type;
  this.client = redis.createClient(); //Create client in each worker constructor
  this.running = true;
  this.job     = null;
}
...
Worker.prototype.getJob = function( fn ) {
  ...
  // this.client - use a client of the worker instance
  this.client.blpop(client.getKey(self.type + ':jobs'), 0, function( err ) {
    if( err || !self.running ) {
