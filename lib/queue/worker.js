...
=======
    client.blpop(client.prefix+':' + self.type + ':jobs', 0, function (err) {
        if (err) return fn(err);		// SAE: Added to avoid crashing redis on zpop
>>>>>>> 6625124... Updates hard-coded 'q' prefix to use redis client prefix
        if (!self.running) {
            return client.lpush(client.prefix+':' + self.type + ':jobs', [1], function () {
                fn(err /*||"shutdown pushback"*/);
            });
        }
        // Set job to a temp value so shutdown() knows to wait
        self.job = true;
        self.zpop(client.prefix+':jobs:' + self.type + ':inactive', function (err, id) {
            if (err || !id) {
                self.job = false;
//                client.lpush('q:' + self.type + ':jobs', [1]);
...
Worker.prototype.shutdown = function (fn, timeout) {
    ...
    var _fn = function (job) {
        ...
        self.client.lpush(self.client.prefix+':' + self.type + ':jobs', [1], function () {});
    ...
    this.once('job failed attempt', _fn);

    if (timeout) {
        shutdownTimer = setTimeout(function () {
            if (self.job) {
                // job is running - fail it and call _fn when complete
