        ...
        job.attempt(function (error, remaining, attempts, max) {
            ...
            if (remaining) {
                ...
                if (job.delay()) {
                    ...
                    //TODO fix delay...
//                    var d = Number( job.delay() );
//                    var backoff = (attempts-1)*d;
//                    d += Date.now() - Number(job.created_at);
////                    d -= backoff;
//                    if( job.backoff() ) console.log( "________________BACKOFF: ", job.backoff()(job, attempts ) );
//                    job.delay(d).update();
                    job.inactive();
