...
Job.prototype.backoff = function (rate,custom_delay,attempts) {
    if (0 == arguments.length) return this._backoff;
    rate = rate || 1; 
    custom_delay = custom_delay || this._delay || 0;    
    attempts = attempts || this._max_attempts || 1;
    this._backoff = custom_delay+":"+rate;
...
Job.prototype.save = function (fn) {
    ...
    this._getId(function(err,id){
        ...
        self.subscribe(function () {
            ...
            var key = 'q:job:' + self.id;
            if (max) self.client.hset(key, 'max_attempts', max);
            if (self._backoff) self.client.hset(key, 'backoff', self._backoff);
