...
//  return search = reds.createSearch(redis.queueName+':search');
//};
...
exports.range = function(from, to, order, fn){
  redis.client().zrange(redis.queueName+':jobs', from, to, get(fn, order));
};
...
exports.rangeByState = function(state, from, to, order, fn){
  redis.client().zrange(redis.queueName+':jobs:' + state, from, to, get(fn, order));
};
...
exports.rangeByType = function(type, state, from, to, order, fn){
  redis.client().zrange(redis.queueName+':jobs:' + type + ':' + state, from, to, get(fn, order));
};
...
exports.get = function(id, fn){
  ...
  job.id = id;
  client.hgetall(redis.queueName+':job:' + job.id, function(err, hash){
    if (err) return fn(err);
...
exports.log = function(id, fn){
  redis.client().lrange(redis.queueName+':job:' + id + ':log', 0, -1, fn);
};
...
Job.prototype.log = function(str){
  ...
  this.client.rpush(redis.queueName+':job:' + this.id + ':log', str);
...
Job.prototype.set = function(key, val, fn){
  this.client.hset(redis.queueName+':job:' + this.id, key, val, fn || noop);
  return this;
};
...
Job.prototype.get = function(key, fn){
  this.client.hget(redis.queueName+':job:' + this.id, key, fn || noop);
  return this;
};
...
//  });
  events.remove(this);
  this.client.del(redis.queueName+':job:' + this.id + ':log', noop);
  this.client.del(redis.queueName+':job:' + this.id, fn || noop);
  return this;
};
...
Job.prototype.removeState = function(fn){
  var client = this.client
    , state = this._state;
  client.zrem(redis.queueName+':jobs', this.id);
  client.zrem(redis.queueName+':jobs:' + state, this.id);
  client.zrem(redis.queueName+':jobs:' + this.type + ':' + state, this.id);
  return this;
};
...
//    var multi = client.multi();
      client.zadd(redis.queueName+':jobs', this._priority, this.id);
      client.zadd(redis.queueName+':jobs:' + state, this._priority, this.id);
      client.zadd(redis.queueName+':jobs:' + this.type + ':' + state, this._priority, this.id);
  // increase available jobs, used by Worker#getJob()
      if('inactive' == state) client.lpush(redis.queueName+':' + this.type + ':jobs', 1);
//    multi.exec(noop);
...
Job.prototype.save = function(fn){
  ...
  // incr id
  client.incr(redis.queueName+':ids', function(err, id){
    if (err) return fn(err);
    var key = redis.queueName+':job:' + id;
    self.id = id;
    self._state = self._state || 'inactive';
    if (max) client.hset(key, 'max_attempts', max);
    client.sadd(redis.queueName+':job:types', self.type);
