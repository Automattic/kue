...
exports.range = function(from, to, order, fn){
  var prefix = require('../kue').prefix;
  pool.alloc().zrange(prefix + ':jobs', from, to, get(fn, order));
};
...
exports.rangeByState = function(state, from, to, order, fn){
  var prefix = require('../kue').prefix;
  pool.alloc().zrange(prefix + ':jobs:' + state, from, to, get(fn, order));
};
...
exports.rangeByType = function(type, state, from, to, order, fn){
  var prefix = require('../kue').prefix;
  pool.alloc().zrange(prefix + ':jobs:' + type + ':' + state, from, to, get(fn, order));
};
...
exports.log = function(id, fn){
  var client = pool.alloc();
  var prefix = require('../kue').prefix;
  client.lrange(prefix + ':job:' + id + ':log', 0, -1, fn);
};
...
Job.prototype.remove = function(fn){
  ...
  var prefix = require('../kue').prefix;
  this.client.del(prefix + ':job:' + this.id, fn || noop);
  return this;
};
