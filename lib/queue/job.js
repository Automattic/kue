...
function getSearch() {
    ...
    var prefix = reds.createClient().prefix
    return search = reds.createSearch(prefix+':search');
}
...
exports.range = function (from, to, order, fn) {
    redis.client().zrange(redis.client().prefix+':jobs', from, to, get(fn, order));
};
...
exports.rangeByState = function (state, from, to, order, fn) {
    redis.client().zrange(redis.client().prefix+':jobs:' + state, from, to, get(fn, order));
};
...
exports.rangeByType = function (type, state, from, to, order, fn) {
    redis.client().zrange(redis.client().prefix+':jobs:' + type + ':' + state, from, to, get(fn, order));
};
...
exports.get = function (id, fn) {
  var client = redis.client()
    ...
    job.id = id;
    client.hgetall(client.prefix+':job:' + job.id, function (err, hash) {
        if (err) return fn(err);
...
exports.removeBadJob = function (id, hash) {
    var client = redis.client();
    if( hash && hash.state ) {
        client.zrem(client.prefix+':jobs:' + hash.state, id);
    }
    client.multi()
        .del(client.prefix+':job:' + id + ':log')
        .del(client.prefix+':job:' + id)
        .zrem(client.prefix+':jobs', id)
    .exec();
...
exports.log = function (id, fn) {
    /*redis*/Job.client/*()*/.lrange(Job.client.prefix+':job:' + id + ':log', 0, -1, fn);
};
...
Job.prototype.log = function (str) {
    ...
    this.client.rpush(this.client.prefix+':job:' + this.id + ':log', str);
...
Job.prototype.set = function (key, val, fn) {
    this.client.hset(this.client.prefix+':job:' + this.id, key, val, fn || noop);
    return this;
};
...
Job.prototype.get = function (key, fn) {
    this.client.hget(this.client.prefix+':job:' + this.id, key, fn || noop);
    return this;
};
...
Job.prototype.remove = function (fn) {
    ...
    this.removeState(function (err) {
        client.multi()
            .del(client.prefix+':job:' + this.id + ':log')
            .del(client.prefix+':job:' + this.id)
            .zrem(client.prefix+':jobs', this.id)
        .exec( function( err ){
//            events.remove(this);
            if( !exports.disableSearch ){
...
Job.prototype.removeState = function (fn) {
    ...
    if( state && state != '' ) {
        client.multi()
            .zrem(client.prefix+':jobs:' + state, this.id)
            .zrem(client.prefix+':jobs:' + this.type + ':' + state, this.id)
        .exec( fn );
    } else {
        fn && fn();
    }
...
Job.prototype.state = function (state, fn) {
    ...
    this.set('state', state, function(){
        var multi = client.multi()
            .zadd(client.prefix+':jobs:' + state, this._priority, this.id)
            .zadd(client.prefix+':jobs:' + this.type + ':' + state, this._priority, this.id);
        ('inactive' == state) ? multi.lpush(client.prefix+':' + this.type + ':jobs', 1) : '';
        multi.exec(function (err, replies) {
            this.set('updated_at', Date.now());
            fn( err );
            //  increase available jobs, used by Worker#getJob()
//                ('inactive' == state) ? client.lpush(client.prefix+':' + this.type + ':jobs', 1, fn) : fn();
        }.bind(this));
    }.bind(this));
//        }.bind(this));
//    }.bind(this));
...
Job.prototype.save = function (fn) {
    ...
    // incr id
    client.incr(client.prefix+':ids', function (err, id) {
        if (err) return fn(err);
        // add the job for event mapping
        var key = client.prefix+':job:' + id;
        self.id = id;
        self.subscribe(function () {
            self._state = self._state || 'inactive';
            if (max) client.hset(key, 'max_attempts', max);
            client.sadd(client.prefix+':job:types', self.type);
...
Job.prototype.update = function (fn) {
    ...
    this.client.zadd(this.client.prefix+':jobs', this._priority, this.id);
