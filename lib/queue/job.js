...
Job.prototype.lockKey = function () {
  var type = this.type,
      ...
      hash;
  if(value){
    hash = ["unique", type, field, value].join(":") ;
  }

  return hash;
};

Job.prototype.lock = function (cb) {
  var Warlock = this.warlock;
  var key = this.lockKey();
  var ms = new Date().getTime();
  if(key){
    Warlock.lock(key, ms, cb);
  } else {
    cb();
  }
...
Job.prototype.unlock = function (cb) {
  var key = this.lockKey();
  var id = this.lockid;
  this.warlock.unlock(key, id, cb);
};
