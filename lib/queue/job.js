...
Job.prototype.state = function(state){
  ...
  // Overload function as getter and setter
  if ("undefined" == typeof state) return this._state;

  var client = this.client;

  // Add to expiration queue
  if ('T' == this.expires && state != this._state && ('failed' == state || 'complete' == state))
  ...
  if ("undefined" != typeof this.data.key)
    client.srem('q:outstanding:' + this.type, this.data.key);
...
Job.prototype.update = function(fn){
  ...
  // search data
  var fxn = function(cb) { cb(null); };
  if ("undefined" != typeof this.data.key)
    fxn = function(cb) { redis.client().sadd('q:outstanding:' + self.type, self.data.key, cb); }
