...
/**
 * Save the job, optionally invoking the callback `fn(err)`. You may also pass
 * `idCallback(id)` to receive the job id with a callback.
 *
 * @param {Function} fn
 * @return {Job} for chaining
 * @api public
...
Job.prototype.save = function(fn, idCallback){
  var client = this.client
    , fn = fn || noop
    , idCallback = idCallback || noop
    , max = this._max_attempts
    , self = this;
