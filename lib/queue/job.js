...
function Job(type, data, options) {
    ...
    if( this.options.searchKeys && !_.isArray( this.options.searchKeys ) ) {
        this.options.searchKeys = [this.options.searchKeys];
    }
    this.type = type;
    this.data = data || {};
...
Job.prototype.update = function (fn) {
    ...
    if( !exports.disableSearch ) {
        if( this.options && this.options.searchKeys ) {
            this.options.searchKeys.forEach( function( key ){
