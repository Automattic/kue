...
var EventEmitter = require('events').EventEmitter
  ...
  , util         = require('util')
  , noop         = function() {}
  , UNIQUE_ALL_FIELDS = ' '; // odds of collision with empty string are better than an arbitrary key
...
/**
 * Set job uniqueness by job type and key value pairs.
 *
 * @param {String} field
 * @return {Job} for chaining
 * @api public
...
Job.prototype.unique = function (field) {
  if(typeof field === 'string'){
    this._unique = field;
  } else {
    this._unique = UNIQUE_ALL_FIELDS;
  }
  return this;
};
...
Job.prototype.remove = function( fn ) {
  ...
  client.multi()
    ...
    .hdel(client.getKey('jobs:unique'), this.uniqKey || '__no_unique_key__')
...
Job.prototype.save = function( fn ) {
  ...
  client.incr(client.getKey('ids'), function( err, id ) {
    ...
    // unique id?
    if( self._unique ) return this.saveUnique(id, fn);

    self.saveWithId(id, fn);
  }.bind(this));
  return this;
};
 ...
 * Save the job, optionally invoking the callback `fn(err)`.
 *
 * @param {number} incrId - The next id from the incrementing counter. It will only be used if there isn't already a known id
 * @param {Function} fn
 * @return {Job} for chaining
 * @api public
 */

Job.prototype.saveUnique = function( incrId, fn ) {
    var client = this.client
      , uniqueKey = {}
      , self = this;

  // TODO: This should include the job type i think
  if (this._unique === UNIQUE_ALL_FIELDS) {
    uniqueKey = this.data;
  } else {
    uniqueKey[this._unique] = this.data[this._unique];
  }

  uniqueKey = [this.type, JSON.stringify(uniqueKey)].join(':');
  this.uniqKey = uniqueKey;

  // Make note of the new unique key, but use the existing jobId if it exists
  client.hsetnx(client.getKey('jobs:unique'), uniqueKey, incrId, function ( err, existed ) {
    if( err ) return fn(err);
    if( existed === 0 ) return fn(); // TODO: response indicating it was previously enqueued?

    self.saveWithId(incrId, fn); // save the id, using the old one if it existed
  });
};
...
Job.prototype.saveWithId = function( id, fn ) {
  var client = this.client
    , fn = fn || noop
    , max = this._max_attempts
    , self = this;
  ...
  if( self.uniqKey ) self.set('uniqKey', self.uniqKey); // store for cleanup upon #remove()

  self.subscribe(function () {
    self._state = self._state || (this._delay ? 'delayed' : 'inactive');
    if (max) {
      self.set('max_attempts', max);
    }
