...
/**
 * Subscribe job by state
 * @param {String} state
 * @param {Function} fn
* @api public
 */
exports.subscribeByState = function( state, fn ) {
  var client = redis.createClient();
  var rangeByState = exports.rangeByState.bind(exports);
  client.on('message', function (channel, message) {
    if(message !== 'zadd')return;
    rangeByState(state,-1,-1,'asc',function( err, jobs ) {
      if( err ) throw Error(err.message);
      var job = jobs[0];
      if(job){
        job.state('inemit',function(err){
          if(err)throw err;
          fn.call(this,job);
        });
      }
  ...
  client.subscribe('__keyspace@0__:q:jobs:'+state);
};
