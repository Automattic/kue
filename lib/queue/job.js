...
/**
 * Initialize a new `Job` with the given `type`, `data` and `id`.
 *
 * @param {String} type
 * @param {Object} data
 ...
 *
 * @param {Boolean} overRide - Skips update function so jobs can be saved with custom ids.
 * @param {Function} fn
 * @return {Job} for chaining
 * @api public
 */

Job.prototype.save = function( overRide, fn ) {
  var client = this.client
    , fn     = fn || noop
  ...
  // update
  if( this.id && !overRide) return this.update(fn);

  // incr id
  client.incr(client.getKey('ids'), function( err, incId ) {
    var id;
    if(overRide){
      id = self.id || incId;
    }
