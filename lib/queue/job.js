...
function Job(type, data, options) {
    ...
    if( this.options.index_keys && !_.isArray( this.options.index_keys ) ) {
        this.options.index_keys = [this.options.index_keys];
    }
    this.type = type;
    this.data = data || {};
...
Job.prototype.update = function (fn) {
    ...
    if( !exports.disableSearch ) {
        if( this.options && this.options.index_keys ) {
            this.options.index_keys.forEach( function( key ){
                getSearch().index(_.deepGetValue(this.data,key)+'', this.id);
            }.bind(this));
        } else {
