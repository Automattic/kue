...
/**
 * Get jobs by created date, with the range `from`..`to`
 * and invoke callback `fn(err, ids)`.
 *
 * @param {Number} from
 * @param {Number} to
...
exports.rangeByCreated = function( from, to, order, fn ) {
  redis.client().zrangebyscore(redis.client().getKey('jobs:created_at'), from, to, get(fn, order));
};
...
exports.removeBadJob = function( id, jobType) {
  ...
  client.multi()
    ...
    .zrem(client.getKey('jobs:created_at'), zid)
...
Job.prototype.remove = function( fn ) {
  ...
  client.multi()
    ...
    .zrem(client.getKey('jobs:created_at'), this.zid)
...
Job.prototype.update = function( fn ) {
  ...
  // add the job to a zset and score by created date. This allows a timestamp based query for jobs.
  this.client.zadd(this.client.getKey('jobs:created_at'), this.created_at, this.zid);
