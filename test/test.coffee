...
describe 'Kue Tests', ->
  ...
  describe 'Job Producer', ->
    ...
    it 'should save jobs having a new id', (done) ->
      job = jobs.create('email-to-be-saved')
    ...
    it 'should set worker id on job hash', (done) ->
      jobs.create('worker-id-test').save()
      jobs.process 'worker-id-test', (job, jdone) ->
        jdone()
        Job.get job.id, (err, j) ->
          j.toJSON().should.have.property 'workerId'
          done()
    ...
    it 'should receive job complete event', (done) ->
      jobs.process 'email-to-be-completed', (job, jdone) ->
        jdone()
      jobs.create('email-to-be-completed')
        .on 'complete', -> done()
        .save()
    ...
    it 'should receive job result in complete event', (done) ->
      ...
      jobs.create('email-with-results')
        .on 'complete', (result) ->
    ...
    it 'should receive job progress event', (done) ->
      jobs.process 'email-to-be-progressed', (job, jdone) ->
        job.progress 1, 2
        jdone()
      jobs.create('email-to-be-progressed')
    ...
    it 'should receive job failed attempt events', (done) ->
      ...
      jobs.process 'email-to-be-failed', (job, jdone) ->
        jdone errorMsg
      jobs.create('email-to-be-failed').attempts(2)
    ...
    it 'should receive queue level complete event', (done) ->
      ...
      testJob = jobs.create('email-to-be-completed').save()
    ...
    it 'should receive queue level failed attempt events', (done) ->
      ...
      newJob = jobs.create('email-to-be-failed').attempts(2).save()
  ...
  describe 'Job', ->
    ...
    it 'should update promote_at after failure with backoff', (done) ->
      now = Date.now()
      job = jobs.create( 'simple-delayed-job-2', { title: 'simple delay job' } )
        .delay(100).attempts(2).backoff({delay: 100, type: 'fixed'}).save()
      calls = 0
    ...
    it 'should receive promotion event', (done) ->
      ...
      jobs.create('email-to-be-promoted').delay(200)
        .on 'promotion', -> done()
        .save()
    ...
    it 'should be re tried after failed attempts', (done) ->
      [total, remaining] = [2, 2]
      jobs.create( 'simple-multi-attempts-job', { title: 'simple-multi-attempts-job' } )
        .attempts(total).save()
